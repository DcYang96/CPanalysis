...
load(file="res_imp_rate.RData")
load(file="res_rf_rate.RData")

load(file="result_rate.RData")
load(file="error.cv_rate.RData")
...
tt <-tt + geom_bar(data = dat ,mapping=aes(dat$name,dat$`%IncMSE`),stat = "identity",col="royalblue3",fill="#87CEFA",width = 1)#+geom_text(data=dat1,aes(label=z),vjust=1.5)

tt <-tt + geom_bar(data = dat ,mapping=aes(dat$name,-dat$IncNodePurity),stat = "identity",col="brown2",fill="#FFDAB9",width = 1)
tt <- tt+xlab(NULL)+ylab(NULL)+coord_flip()

tt <-tt +scale_x_continuous(expand = c(0,0),breaks  =NULL)+scale_y_continuous(expand = c(0,0),breaks =seq(-1,1,0.2), labels =abs(seq(-1,1,0.2)) ,limits = c(-1,1))
...
pdf("FigureS1A.pdf",width = 7,height = 4)
tt
