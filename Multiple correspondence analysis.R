...
summary(new_dat)
res.mca <- MCA(new_dat[,c(1:6)], graph = T)
...
dat3 <- read_excel("Chinese_data.xlsx",sheet=4)

dat1 <- as.data.frame(dat1)
dat2 <- as.data.frame(dat2)
dat3 <- as.data.frame(dat3[,c(1,4)])
...
cl3 <- cut(as.numeric(dat3$`population density`),include.lowest=T,quantile(dat3$`population density`,c(0,0.33,0.66,1)),labels=c("L_pd","M_pd","H_pd"))
...
for (i in 1:15) {
  k=rowSums(dat[i,3:6])
  city1 <- matrix(NA,nrow = k,ncol = 6)
  city1[,1] <- dat[i,1]
  city1[,2] <- dat[i,2]
  city1[,4] <- n1[cl1[which(names(clusters1)==dat[i,1])]]
  city1[,5] <- n2[cl2[which(names(clusters2)==dat[i,1])]]
  city1[,6] <- as.character( cl3[which(names(clusters2)==dat[i,1])])

  class(cl3[which(names(clusters2)==dat[i,1])])
...
colnames(rdat) <- c("City","Reigon","Type","Transportation capacity","Intensive rearing degree","Population density")
library("FactoMineR") 
library("factoextra")
res.mca <- MCA(rdat[,c(2:6)], graph = T)
tt <- fviz_mca_var(res.mca, 
  repel = TRUE,ggtheme = theme_minimal() )+ theme_bw() +
  theme(panel.grid.major=element_line(colour=NA),
    panel.background = element_rect(fill ="transparent",colour = NA),
